...
const AnnualTokenTax = ({ open, user }) => {
  ...
  const uploadCompressedFile = async (file, normalizedFileName, token) => {
    ...
    try {
      ...
      xhr.open('POST', `${process.env.REACT_APP_API_BASE_URL}/files/vehicles-tokentaxes-annualtokentax/${encodeURIComponent(selectedZone)}/${encodeURIComponent(selectedBranch)}`);
