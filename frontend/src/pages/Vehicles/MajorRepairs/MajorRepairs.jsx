...
const MajorRepairs = ({ open, user }) => {
  ...
  const fetchFiles = useCallback(async (forceRefresh = false) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_BASE_URL}/files/vehicles-majorrepairs/${encodedZone}/${encodedBranch}`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  const uploadSingleFile = useCallback(async (file, progressCallback) => {
    ...
    try {
      ...
      return new Promise((resolve, reject) => {
        ...
        xhr.open('POST', `${process.env.REACT_APP_API_BASE_URL}/files/vehicles-majorrepairs/${encodeURIComponent(selectedZone)}/${encodeURIComponent(selectedBranch)}`);
  ...
  const handleDeleteConfirm = useCallback(async () => {
    ...
    try {
      ...
      const deleteUrl = `${process.env.REACT_APP_API_BASE_URL}/files/vehicles-majorrepairs/${encodeURIComponent(selectedZone)}/${encodeURIComponent(selectedBranch)}/${encodedFilename}`;
      ...
      <Box sx={{ width: '100%', backgroundColor: theme.palette.mode === 'dark' ? '#333' : '#fff', padding: '20px', borderRadius: '8px', transition: 'background-color 0.3s' }}>
        ...
        <Box sx={{ width: '100%', maxHeight: '500px', overflowY: files.length > 5 ? 'scroll' : 'unset', backgroundColor: theme.palette.mode === 'dark' ? '#222' : '#fafafa', color: theme.palette.mode === 'dark' ? '#f5f5f5' : '#333', padding: '20px', borderRadius: '8px', transition: 'background-color 0.3s, color 0.3s' }}>
          {loading ? (
            ...
            <FileTable files={filteredFiles} onDelete={openDeleteDialog} onView={handleViewFile} user={user} filePathPrefix="VEHICLES/MAJORREPAIRS" showFileNumber={true} />
          )}
        </Box>
      </Box>
...
export default React.memo(MajorRepairs);